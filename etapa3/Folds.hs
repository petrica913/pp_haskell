...
instance Show TransformationAST where
    show (T transformation) =
        case transformation of
            Translation tx ty -> "+(" ++ show tx ++ "," ++ show ty ++ ")" -- show tx si show ty sunt deja stringuri
            Scaling f -> "*<" ++ show f ++ ">" -- show f este deja un string
            Combine transformations -> show transformations -- transformations este deja un string
...
helper :: Int -> RegionAST -> String
helper level (R regionShape) =
    let indent = replicate (2 * level) ' ' -- 2 spatii pentru fiecare nivel
    in case regionShape of
        ...
        Transform transformation region -> indent ++ show transformation ++ "\n" ++ helper (level + 1) region
        Complement region -> indent ++ "~\n" ++ helper (level + 1) region
        Union region1 region2 -> indent ++ "+\n" ++ helper (level + 1) region1 ++ "\n" ++ helper (level + 1) region2
...
instance Num RegionAST where
    ...
    -}
    region1 - region2 = region1 * complement region2 -- intersectie cu complementul
...
instance Functor TransformationShape where
    ...
    fmap f transformation = -- f este o functie care primeste un a si intoarce un b
        case transformation of
            Translation tx ty -> Translation tx ty
            Scaling f -> Scaling f
            Combine transformations -> Combine $ fmap f transformations -- aplicam f pe fiecare element din lista transformations
...
-- f este o functie care primeste un TransformationShape a si intoarce un a

foldRegionAST :: RegionCombiner a -> RegionAST -> a
foldRegionAST f (R region) = f (fmap (foldRegionAST f) region)
-- f este o functie care primeste un RegionShape a si intoarce un a
...
-- combiner este o functie care primeste un TransformationShape
-- Transformation si intoarce un Transformation
  where
...
-- combiner este o functie care primeste un TransformationShape Int si intoarce un Int
  where
...
{-
    *** TODO ***
    ...
    pentru că, în momentul în care evaluăm doar o componentă
    a perechii, Haskell nu va evalua și cealaltă componentă. Astfel, putem
    accesa doar componenta dorită fără a evalua și cealaltă componentă.
    În cazul în care am fi evaluat ambele componente, am fi pierdut din eficiență
...
-- combiner este o functie care primeste un RegionShape (Int, Int) si intoarce un (Int, Int)
  where
    ...
    combiner region = 
        case region of
            ...
            Rectangle _ _ -> (1, 0)
            Circle _ -> (1, 0)
            Transform t (countR, countT) -> (countR, countT + basicTransformationCount t)
            Complement (countR, countT) -> (countR, countT)
            Union (countR1, countT1) (countR2, countT2) -> (countR1 + countR2, countT1 + countT2)
            Intersection (countR1, countT1) (countR2, countT2) -> (countR1 + countR2, countT1 + countT2)
